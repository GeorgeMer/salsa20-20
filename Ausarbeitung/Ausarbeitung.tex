...
\\
Die Korrektheit unserer Implementationen des Salsa20/20-Verfahrens hängt von der Korrektheit der Kryptfunktion und der Kernfunktion, sowie vom Einlesen des Schlüssels ab. (Das Einlesen der Nonce nutzt Funktionen der Standard-Bibliothek, von denen wir annehmen können, dass sie korrekt sind).

\subsection{Einlesen des Schlüssels}
Beim Einlesen ist es irrelevant, ob eine Hexzahl oder einen String eingeben wird, da sie auf die gleiche Weise interpretiert werden, 
auch wenn sie einen etwas anderen Algorithmus erfordern, um sie in ein Array von \texttt{\textcolor{blue}{uint32\_t}} zu parsen. 
Wie das Parsen des Keys funktioniert wird in Abschnitt \ref{main} genauer erläutert.
Um dann die Korrektheit der geparsten Eingabe zu überprüfen, müssen nur einige repräsentative Unit-Tests geschrieben werden, 
...
Es wurden einige Testfälle angefertigt, die die Ergebnisse zwischen gleichwertigen Eingaben jeweils hexadezimal und als String vergleichen.


\subsection{Die Kryptfunktion}
Die Kryptfunktion \texttt{salsa20\_crypt} nutzt XOR-Operationen, um die Nachricht mit der Ausgabe der Kernfunktion \texttt{salsa20\_core} zu verschlüsseln.
...
Um also die Korrektheit der Kryptfunktion zu testen, müssen nur einige repräsentative, manuelle
und viele automatisierte Zufallstests geschrieben werden, die die Kryptfunktion zweimal aufrufen.
Beim ersten Aufruf ist dann die Eingabe ein Array, das die zu verschlüsselnde Nachricht enthält und die Ausgabe ist eine Chiffre. 
Beim zweiten Aufruf ist die Eingabe diese Chiffre und die Ausgabe ein anderes Array. 
Am Ende vergleichen wir das ursprüngliche Array mit dem Array, das nach dem zweiten 
...
Der Aufruf dieser Referenzimplementierung für eine Menge von Eingaben und auch eine der Versionen unserer Implementierung mit denselben Eingaben ergibt dieselbe Chiffre, wenn die Version tatsächlich korrekt implementiert wurde. 
...
% TODO: key random generieren und ausgeben oder mit passwort umsetzen
